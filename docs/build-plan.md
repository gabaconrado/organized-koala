...
| 0005 | TUI — responsive (non-blocking) event loop + `tui::app` submodule reorg | Foundation | in-flight (branch-owned) | ADR-0006 Model A (worker thread + mpsc + polled loop, no async); TUI-only; reviewed + verified at code-hash `bc89672…`; live status on `feature/0005-tui-responsive-event-loop` |
