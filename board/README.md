...
| [0002](./features/0002-contract-crate.md) | Contract crate + workspace restructure (slice 1 of 0001) | awaiting-merge (branch-owned) | high | — | feature/0002-contract-crate |
...
> is the umbrella tracking the three. `0002` is **branch-owned** on `feature/0002-contract-crate`
> at AI-terminal `awaiting-merge` — its live status lives on the branch; the human merges that
> branch (→ `merged`), which brings the finished item back to `main`. `0002` was **re-opened**
> after first reaching `awaiting-merge` by two `[human]` feedback items (chrono-typed timestamps;
> a test-layout question), both **re-resolved** on-branch (chrono implemented + re-reviewed
> `98d1a85` + re-verified; test-layout was a clarification plus a `rust-standards` rule) — so it
> returns to `awaiting-merge`. `0003` is unblocked and ready to claim once 0002 merges.
